...
class ByteBufferMessageSet(val buffer: ByteBuffer) extends MessageSet with Logging {
  ...
  def this(messages: Message*) {
    this(ByteBufferMessageSet.create(new AtomicLong(0), NoCompressionCodec, messages: _*))
  }
   ...
   */
  private def assignOffsets(offsetCounter: AtomicLong, codec: CompressionCodec): ByteBufferMessageSet = {
    if (codec == NoCompressionCodec) {
      ...
      new ByteBufferMessageSet(compressionCodec = codec,
