...
import org.dist.queue.{KafkaException, Logging, SystemTime, Time, Utils}
 ...
 */
private[queue]abstract class AbstractServerThread extends Runnable with Logging {
 ...
 */
private[queue]class Acceptor(val host: String, val port: Int, private val processors: Array[Processor],
 ...
 */
private[queue]class Processor(val id: Int,
                               val time: Time,
                               val maxRequestSize: Int,
                               val requestChannel: RequestChannel) extends AbstractServerThread with Logging {
    ...
    while(curr != null) {
      ...
      try {
        curr.responseAction match {
          ...
          case responseCode => throw new RuntimeException("No mapping found for response code " + responseCode)
        }
      } catch {
