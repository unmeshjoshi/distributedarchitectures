...
class BoundedByteBufferSend(val buffer: ByteBuffer) extends Send {
  ...
  private var sizeBuffer: ByteBuffer = ByteBuffer.allocate(4)

  // Avoid possibility of overflow for 2GB-4 byte buffer
  //FIXME ambiguity between Int.limig and ByteBuffer.limit
...
//                                       "allowable size for a bounded buffer is " + (Int.MaxValue - sizeBuffer.limit) + ".")
  ...
  def this(request: RequestOrResponse) = {
    this(request.serialize().size)
    buffer.put(request.serialize().getBytes)
    buffer.rewind()
   }
