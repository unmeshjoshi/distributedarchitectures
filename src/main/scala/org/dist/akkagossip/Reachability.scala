...
object Reachability {
  ...
  def apply(records: immutable.IndexedSeq[Record], versions: Map[InetAddressAndPort, Long]): Reachability =
    new Reachability(records, versions)

  def create(records: immutable.Seq[Record], versions: Map[InetAddressAndPort, Long]): Reachability = records match {
    case r: immutable.IndexedSeq[Record] => apply(r, versions)
    case _                               => apply(records.toVector, versions)
  }

  @SerialVersionUID(1L)
  final case class Record(observer: InetAddressAndPort, subject: InetAddressAndPort, status: ReachabilityStatus, version: Long)

  trait ReachabilityStatus
...
class Reachability private (
                                              ...
                                              val versions: Map[InetAddressAndPort, Long])
  ...
  private class Cache {
    ...
    val (observerRowsMap, allUnreachable, allTerminated) = {
      if (records.isEmpty) {
        val observerRowsMap = Map.empty[InetAddressAndPort, Map[InetAddressAndPort, Reachability.Record]]
        val allTerminated = Set.empty[InetAddressAndPort]
        val allUnreachable = Set.empty[InetAddressAndPort]
        (observerRowsMap, allUnreachable, allTerminated)
      } else {
        val mapBuilder = scala.collection.mutable.Map.empty[InetAddressAndPort, Map[InetAddressAndPort, Reachability.Record]]
        var allTerminated = Set.empty[InetAddressAndPort]
        var allUnreachable = Set.empty[InetAddressAndPort]
        ...
        val observerRowsMap: Map[InetAddressAndPort, Map[InetAddressAndPort, Reachability.Record]] = mapBuilder.toMap

        (observerRowsMap, allUnreachable.diff(allTerminated), allTerminated)
      }
    }

    val allUnreachableOrTerminated: Set[InetAddressAndPort] =
  ...
  private def observerRows(observer: InetAddressAndPort): Option[Map[InetAddressAndPort, Reachability.Record]] =
    cache.observerRowsMap.get(observer)

  def unreachable(observer: InetAddressAndPort, subject: InetAddressAndPort): Reachability =
    change(observer, subject, Unreachable)

  def reachable(observer: InetAddressAndPort, subject: InetAddressAndPort): Reachability =
    change(observer, subject, Reachable)

  def terminated(observer: InetAddressAndPort, subject: InetAddressAndPort): Reachability =
    change(observer, subject, Terminated)

  private def currentVersion(observer: InetAddressAndPort): Long = versions.get(observer) match {
    case None    => 0
    case Some(v) => v
  }

  private def nextVersion(observer: InetAddressAndPort): Long = currentVersion(observer) + 1

  private def change(observer: InetAddressAndPort, subject: InetAddressAndPort, status: ReachabilityStatus): Reachability = {
  ...
  def merge(allowed: immutable.Set[InetAddressAndPort], other: Reachability): Reachability = {
  ...
  def remove(nodes: Iterable[InetAddressAndPort]): Reachability = {
    val nodesSet: Set[InetAddressAndPort] = nodes.toSet
  ...
  def removeObservers(nodes: Set[InetAddressAndPort]): Reachability =
    if (nodes.isEmpty)
      this
    else {
  ...
  def status(observer: InetAddressAndPort, subject: InetAddressAndPort): ReachabilityStatus =
    observerRows(observer) match {
      case None => Reachable
  ...
  def status(node: InetAddressAndPort): ReachabilityStatus =
   ...
   */
  def isReachable(node: InetAddressAndPort): Boolean = isAllReachable || !allUnreachableOrTerminated.contains(node)

  /**
   * @return true if the given observer node can reach the subject node.
   */
  def isReachable(observer: InetAddressAndPort, subject: InetAddressAndPort): Boolean =
   ...
   */
  def allUnreachable: Set[InetAddressAndPort] = cache.allUnreachable
   ...
   */
  def allUnreachableOrTerminated: Set[InetAddressAndPort] = cache.allUnreachableOrTerminated
   ...
   */
  def allUnreachableFrom(observer: InetAddressAndPort): Set[InetAddressAndPort] =
    observerRows(observer) match {
      case None => Set.empty
      case Some(observerRows) =>
  ...
  def observersGroupedByUnreachable: Map[InetAddressAndPort, Set[InetAddressAndPort]] = {
    records.groupBy(_.subject).collect {
      case (subject, records) if records.exists(_.status == Unreachable) =>
        val observers: Set[InetAddressAndPort] =
          records.iterator.collect { case r if r.status == Unreachable => r.observer }.to(immutable.Set)
        subject -> observers
    }
  }

  def allObservers: Set[InetAddressAndPort] = records.iterator.map(_.observer).toSet

  def recordsFrom(observer: InetAddressAndPort): immutable.IndexedSeq[Record] = {
    observerRows(observer) match {
  ...
  override def toString: String = {
    val rows = for {
      ...
      s"${observer.getAddress} -> ${subject.getAddress}: ${record.status} [$aggregated] (${record.version})"
    }
